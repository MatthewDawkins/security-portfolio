...
function noise(input: number): number {
...
HTMLCanvasElement.prototype.toDataURL = function (type?: string, quality?: number): string {
  if (this.width === 0 || this.height === 0) return _toDataURL.call(this, type, quality)

  const off = document.createElement('canvas')
  off.width  = this.width
  off.height = this.height
  const ctx  = off.getContext('2d')
  if (!ctx) return _toDataURL.call(this, type, quality)
