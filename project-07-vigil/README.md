...
Vigil reads a JSONL log file, evaluates every event against a directory of YAML detection rules, and produces both a live terminal alert table and an HTML report. It ships with 15 production-tuned rules covering the most common Linux attack patterns from initial access through persistence.
...
|---|---|---|---|---|
| vigil-001 | SSH Brute Force | threshold | high | T1110.001 |
| vigil-002 | Password Spray Attack | threshold | high | T1110.003 |
| vigil-003 | Repeated Sudo Failure | threshold | medium | T1548.003 |
| vigil-004 | New Local User Account Created | match | medium | T1136.001 |
| vigil-005 | Cron Job Added or Modified | match | medium | T1053.003 |
| vigil-006 | Sensitive Credential File Accessed | match | high | T1003.008 |
| vigil-007 | Suspicious Process Execution | match | high | T1059.004 |
| vigil-008 | Web Application Auth Brute Force | threshold | medium | T1110 |
| vigil-009 | Potential C2 Beacon | threshold | medium | T1071.001 |
| vigil-010 | Multiple Account Lockouts | threshold | medium | T1110 |
| vigil-011 | SSH Lateral Movement Chain | sequence | critical | T1021.004 |
| vigil-012 | Privilege Escalation via Sudo | sequence | high | T1548.003 |
| vigil-013 | Credential Dump Pattern | sequence | critical | T1003.008 |
| vigil-014 | Recon to Exploitation Chain | sequence | high | T1046 |
| vigil-015 | Persistence Backdoor Chain | sequence | high | T1136.001 |

---

## Demo Output

Against the included `logs/demo_attack.log` (an 81-event, 15-phase attack simulation), Vigil fires **25 alerts** spanning every tactic from initial access through persistence:
...
rules/
    vigil-001-*.yml     # 15 detection rules (match / threshold / sequence)
...
reports/
    vigil-demo.html       # Pre-generated HTML detection report
```
...
python vigil.py scan logs/demo_attack.log

# Scan with HTML report output
python vigil.py scan logs/demo_attack.log --output reports/my-report.html

# Use a custom rules directory
python vigil.py scan logs/my-app.log --rules /path/to/rules --output report.html

# List all loaded rules
python vigil.py rules
...
Vigil expects JSONL (one JSON object per line). The only required field is `timestamp` (ISO-8601). All other fields are arbitrary — rule selections match against whatever keys are present in each event.
...
2. Set a unique `id` (e.g. `vigil-016`)
